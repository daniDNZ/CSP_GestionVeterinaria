...
import { getDataDatalist, handleDatalist } from "./Datalist";
...
function SearchFilter({ arr, fetchMethod }) {
    ...
    const findData = (filter, arr) => {

        if (arr[0] == 'patients') {
            findPatients(assignData, filter, arr);
            findPatients(getData, filter, arr);
        }
        if (arr[0] == 'customers') {
            findCustomers(assignData, filter, arr);
            findCustomers(getData, filter, arr);
        }
        if (arr[0] == 'visits') {
            findVisits(assignData, filter, arr);
            findVisits(getData, filter, arr);
        }
...
// const activePagination = (data, ids, filter = {}) => {
//     // Pagination
